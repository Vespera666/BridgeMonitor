...
void MonitorPointManageDialog::initUI()
{
    ...
    connect(m_cbxPointSelect, &QComboBox::currentTextChanged, this, &MonitorPointManageDialog::slotPointComboboxChanged);
}
...
bool MonitorPointManageDialog::saveMonitorPoint(const MonitoringPoint &mp, bool isUpdate, const QString &oldPointId)
{
    QFile f(MONITOR_FILE);
    if(!f.open(QIODevice::ReadOnly|QIODevice::Text))
    {
    ...
    if(isUpdate && !oldPointId.isEmpty())
    {
        ...
        for(QString l : lines)
        {
            ...
            if(col.size()>=1 && col[0]==oldPointId) continue;
            newLines.append(l);
        }
        lines = newLines;
    }
    QString newLine = QString("%1,%2,%3")
                          .arg(mp.pointId).arg(mp.sectionName).arg(mp.installDate.toString("yyyy-MM-dd"));
...
QStringList MonitorPointManageDialog::loadAllSensorModel()
{
    ...
    while(!st.atEnd())
    {
        ...
        if(col.size()>=1) res.append(col[0]);
    }
    f.close();
    return res;
}
...
void MonitorPointManageDialog::slotDelPoint()
{
    ...
    QMessageBox::StandardButton ret = QMessageBox::question(this,"确认删除",
...
void MonitorPointManageDialog::slotDoBind()
{
    ...
    {
        auto res = QMessageBox::question(this,"警告",
                                         QString("监测点【%1】已绑定传感器，是否替换原有绑定？").arg(pointId),
                                         QMessageBox::Yes|QMessageBox::No);
...
void MonitorPointManageDialog::slotDoUnBind()
{
    ...
    auto res = QMessageBox::question(this,"确认解绑",
