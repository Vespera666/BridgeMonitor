...
void MonitorPointTableModel::loadData(const QVector<MonitoringPoint> &data, const QStringList &bindInfo)
{
    beginResetModel();
    m_points = data;
...
QVariant MonitorPointTableModel::data(const QModelIndex &index, int role) const
{
    ...
    int r = index.row();
    int c = index.column();
    if(r >= m_points.size() || c >= m_header.size()) return {};
    ...
    if(role == Qt::DisplayRole)
    {
        switch(c)
        {
        case 0: return mp.pointId;
        case 1: return mp.sectionName;
        case 2: return mp.installDate.toString("yyyy-MM-dd");
        case 3: return m_bindSensorText[r];
        default: return {};
        }
    }
